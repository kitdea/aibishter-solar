    ...
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pt-32 pb-24 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-8 md:pb-20 px-6 md:px-12 z-10 text-center md:text-left">
          ...
          >
            <div className="flex items-center justify-center md:justify-start gap-2 mb-3 md:mb-6 flex-wrap">
              ...
              <span className="text-white/60 text-xs font-bold uppercase tracking-widest">Aibishter Solar</span>
              <ChevronRight size={14} className="text-white/40" />
              <Link href="/services" className="text-white/60 text-xs font-bold uppercase tracking-widest hover:text-white transition-colors">
                Services
              </Link>
              <ChevronRight size={14} className="text-white/40" />
        ...
        >
          <div className="lg:w-1/2 space-y-8">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs uppercase tracking-widest font-bold text-accent-blue">/ Overview</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white tracking-tight leading-tight">
              What we <span className="text-slate-400 dark:text-slate-500">deliver.</span>
            </h2>
            <p className="text-xl text-slate-500 dark:text-slate-400 font-sans leading-relaxed">{service.description}</p>
            <p className="text-lg text-slate-500 dark:text-slate-400 font-sans leading-relaxed">{service.longDescription}</p>
            <Link
              href="/contact"
            ...
            >
              Request a Quote <ArrowRight className="group-hover:translate-x-1 transition-transform" size={20} />
            </Link>
          </div>
      ...
      <section className="bg-white dark:bg-slate-900 py-24">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          >
            <div className="text-xs uppercase tracking-widest font-bold text-accent-blue mb-4">/ What&apos;s Included</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {(service.features ?? []).map((feature, idx) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white mb-2">{feature.label}</h3>
                  <p className="text-slate-500 dark:text-slate-400 font-sans text-sm leading-relaxed">{feature.detail}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 py-24">
        <motion.div
          ...
          className="text-center mb-16"
        >
          <div className="text-xs uppercase tracking-widest font-bold text-accent-blue mb-4">/ Why Aibishter</div>
          <h2 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white tracking-tight">
            The Aibishter <span className="text-slate-400 dark:text-slate-500">advantage.</span>
          </h2>
        </motion.div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {service.benefits.map((benefit, idx) => (
            <motion.div
              ...
              whileInView="visible"
              viewport={{ once: true, margin: "-60px" }}
              variants={{ hidden: { opacity: 0, scale: 0.97 }, visible: { opacity: 1, scale: 1, transition: { duration: 0.5, delay: idx * 0.1 } } }}
              className={`p-8 rounded-4xl border ${idx === 0 ? "bg-accent-blue border-accent-blue" : "bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700"}`}
            >
              <div className={`text-3xl font-extrabold tracking-tighter mb-4 font-mono ${idx === 0 ? "text-blue-200" : "text-accent-blue opacity-40"}`}>
                0{idx + 1}
              </div>
              <h3 className={`text-xl font-bold mb-3 ${idx === 0 ? "text-white" : "text-slate-900 dark:text-white"}`}>{benefit.title}</h3>
              <p className={`font-sans text-sm leading-relaxed ${idx === 0 ? "text-blue-100" : "text-slate-500 dark:text-slate-400"}`}>{benefit.description}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 pb-8">
        ...
        >
          ...
          <div className="relative z-10">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight font-poppins">Ready to get started?</h2>
            <p className="text-white/70 font-sans mb-10 max-w-lg mx-auto leading-relaxed">
              Contact us today for a free consultation and custom quote on your {service.title.toLowerCase()} project.
            </p>
